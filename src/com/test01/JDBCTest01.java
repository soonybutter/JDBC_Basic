...
public class JDBCTest01 {
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException{
		...
		ResultSet rs =stmt.executeQuery(" SELECT * FROM EMPLOYEE"); //DB서버로 해당 쿼리문을 날림
		
		//3. 결과처리
		// next() : 커서를 다음 row로 이동시킨 후 데이터가 있는지 없는지 판단(있으면true, 없으면 false)
		while(rs.next()) {
			System.out.println(rs.getString(1)+" "+rs.getString(2)+" "+rs.getInt("SALARY"));
			                      //getXXX(): 현재 커서가 가르키는 row의 컬럼값을 가지고 오겠다.
		}
		//4. 종료 : 마지막으로 만들어진것부터 close()
		rs.close();
		stmt.close();
		con.close();
