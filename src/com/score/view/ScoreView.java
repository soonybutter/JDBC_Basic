...
public class ScoreView {
	...
	public static void main(String[] args) {
		...
		while(select != 6) {
			...
			case 1:
				//전체출력
				List<Score> all = biz.getAll(); //db에 저장된 모든 데이터들 다 가져옴
				for(Score s : all) {
			...
			case 4:
				...
				if(check != null) {
					System.out.println("이전 국어 점수: "+check.getKor()+" - 바꿀 국어 점수: ");
					kor = sc.nextInt();
					System.out.println("이전 영어 점수: "+check.getEng() +" - 바꿀 영어 점수: ");
					eng = sc.nextInt();
					System.out.println("이전 수학 점수: "+check.getMath()+" - 바꿀 수학 점수: ");
					math = sc.nextInt();
